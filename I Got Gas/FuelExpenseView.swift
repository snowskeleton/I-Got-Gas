...
struct FuelExpenseView: View {
        ...
        ForEach(cars, id: \.self) { car in
            ...
            VStack {
                ...
                Button("Add Expense") {
                    ...
                    AddExpenseView(carID: car.id ?? "", car: Binding<Car>.constant(cars[0]), isGas: Binding<Bool>.constant(true))
    ...
    func loseMemory(at offsets: IndexSet) {
        for index in offsets {
            ...
            moc.delete(service)
            try? self.moc.save()
            AddExpenseView(carID: cars[0].id ?? "", car: Binding<Car>.constant(cars[0]), isGas: Binding<Bool>.constant(true)).updateCarStats(cars[0])
        }
    }
}
